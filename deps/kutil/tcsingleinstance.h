...
/// @date 2013-04-19
class TcSingleInstance
{
public:
    /// @brief 初始化方法
    /// @param [in] instanceName 识别是否重复开启的实例名称
    TcSingleInstance(const QString& instanceName)
    {
        init(0, instanceName);
    }
    /// @brief 初始化方法
    ...
    /// @param [in] instanceName 识别是否重复开启的实例名称
    TcSingleInstance(QObject* parent, const QString& instanceName)
    {
...
private:
    void init(QObject* parent, const QString& instanceName)
    {
        m_first = false;
